...
> Este archivo es **guía**, no template. Léelo cuando estés definiendo el `stack/` y `repo-config.yaml` del repo. Lo que apliques se materializa en `stack/architecture.md` + `repo-config.yaml`.
